...
class ClientsController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $table->editColumn('status', function ($row) {
                return $row->status ? Client::STATUS_SELECT[$row->status] : '';
            });
            $table->editColumn('language', function ($row) {
                return $row->language ? Client::LANGUAGE_SELECT[$row->language] : '';
            });
    ...
    public function show(Client $client)
    {
        abort_if(Gate::denies('client_show'), Response::HTTP_FORBIDDEN, '403 Forbidden');
         ...
         // Fetch all addresses associated with the client
        $clientAddresses = $client->addresses;
        $orders = $client->orders;

        return view('admin.clients.show', compact('client','clientAddresses','orders'));
    }
