...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.commandLog.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.command-logs.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('box') ? 'is-invalid' : '' }}" name="box_id" id="box_id" required>
                    @foreach($boxes as $id => $entry)
                        <option value="{{ $id }}" {{ old('box_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('box'))
